...
use crate::util::ext_trait::{AsyncExt, ByteSliceExt, Encode, StringExt};
use crate::util::fp_utils;
...
impl RedisConnectionPool {
    ...
    {
        self.pool
            .set(key.tenant_aware_key(self), value, Some(Expiration::KEEPTTL), None, false)
            .await
            .change_context(errors::RedisError::SetFailed)
    }
    ...
    where
        ...
        V::Error: Into<fred::error::RedisError> + Send + Sync,
    {
        self.pool.msetnx(value).await.change_context(errors::RedisError::SetFailed)
    }
    ...
    {
        let serialized =
            value.encode_to_vec().change_context(errors::RedisError::JsonSerializationFailed)?;
        self.set_key_if_not_exists_with_expiry(key, serialized.as_slice(), ttl).await
    }
        ...
        V: serde::Serialize + Debug,
    {
        ...
        self.set_key_without_modifying_ttl(key, serialized.as_slice()).await
    }
    ...
    {
        self.pool
            .set(key.tenant_aware_key(self), value, Some(Expiration::EX(seconds)), None, false)
            .await
            .change_context(errors::RedisError::SetExFailed)
    }
        ...
        V::Error: Into<fred::error::RedisError> + Send + Sync,
    {
        self.pool
            .set(
                key.tenant_aware_key(self),
                value,
                Some(Expiration::EX(seconds.unwrap_or(self.config.default_ttl.into()))),
                Some(SetOptions::NX),
                false,
            )
            .await
        ...
        V::Error: Into<fred::error::RedisError> + Send + Sync,
    {
        ...
        output
            .async_and_then(|inner| async {
                self.set_expiry(key, ttl.unwrap_or(self.config.default_hash_ttl).into()).await?;
                Ok(inner)
            })
            .await
    }
        ...
        V: serde::Serialize + Debug,
    {
        ...
        self.set_hash_field_if_not_exist(key, field, serialized.as_slice(), ttl).await
    }
        ...
        V: serde::Serialize + Debug,
    {
        ...
        for (key, val) in kv {
            hsetnx
                .push(self.serialize_and_set_hash_field_if_not_exist(key, field, val, ttl).await?);
        }
        Ok(hsetnx)
    }
        ...
        Ids: Into<MultipleIDs> + Debug + Send + Sync,
    {
        match group {
            ...
            None => {
                self.pool.xread_map(count, block, self.get_keys_with_prefix(streams), ids).await
            },
        }
        .map_err(|err| match err.kind() {
            RedisErrorKind::NotFound | RedisErrorKind::Parse => {
    ...
    pub async fn consumer_group_create(
        ...
        if matches!(id, RedisEntryId::AutoGeneratedID | RedisEntryId::UndeliveredEntryID) {
